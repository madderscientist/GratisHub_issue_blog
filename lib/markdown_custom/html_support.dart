...
/// Remove non-numeric characters from width and height attributes
void stripWidthHeightUnits(h.Node node) {
  if (node is h.Element) {
    for (var key in ['width', 'height']) {
      if (node.attributes.containsKey(key)) {
        final raw = node.attributes[key]!;
        node.attributes[key] = RegExp(r'\d+(?:\.\d+)?').stringMatch(raw) ?? '';
      }
    }
    for (var child in node.nodes) {
      stripWidthHeightUnits(child);
    }
  }
}
...
List<SpanNode> parseHtml(
  ...
  try {
    ...
    // Sanitize width and height attributes
    for (var node in document.nodes) {
      stripWidthHeightUnits(node);
    }
